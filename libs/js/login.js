...
$(document).ready(function() {
  ...
  $('#formulario_ingreso').submit(function(e) {
    ...
    if (n!=0) {
      ...
      const url='http://localhost/coop/ingresarmaterial/agregarIngresarMaterial';
      ...
      success: function(response) {
        ...
        detalles.forEach(detalle => {
          ...
          <tr detalleid="${detalle.id_temporal}" style="text-align:center;">
            ...
            <td>
              <input type="text" name="valor" class="form-control" value="${detalle.valor}" disabled required>
            </td>
            <td class="subtotal">
...
$('#formulario_detalle_pedido').ready(darDetallePedido); 
  ...
  $('#formulario_pedido').submit(function(e) {
    ...
    if (n!=0) {
      ...
      let template=`
      <div class='alert alert-warning alert-dismissible fade show' role='alert'>
          <strong>Falta Detalle</strong> Llene el Detalle del Pedido Porfavor
      </div>
      `
      $('#response').html(template);
    }
      ...
      success: function(response) {
        ...
        detalles.forEach(detalle => {
          ...
          <tr pedidoid="${detalle.id_temporal}" style="text-align:center;">
            ...
            <td>
              <input type="text" name="precio" class="form-control" value="${detalle.precio}" disabled required>
            </td>
            <td class="subtotal">
               ${detalle.total.toFixed(2)}
            </td>
...
//mostrar tabla
  ...
  $("#id_proveedor").keydown(busquedaFacturaProveedor($("#id_proveedor").val()));
  $("#id_proveedor").change(busquedaFacturaProveedor($("#id_proveedor").val()));
      ...
      $("#id_proveedor").change(function() {
        if ($("#id_proveedor").val()!=inicial) {
          const url="http://localhost/coop/ingresarmaterial/buscarRegistro";
            ...
            url:url,
            type:'GET',
            data: {id_proveedor: $('#id_proveedor').val(),numero_factura: $('#numero_factura').val()},
            success: function(response) {
              if (response==1) {
                enabled();
                //hideAlertaResponse();
    ...
    //PRODUCTO
    $("#numero_factura").keyup(busquedaFacturaProveedorn($("#numero_factura").val()));
    $("#numero_factura").keydown(busquedaFacturaProveedorn($("#numero_factura").val()));
    $("#numero_factura").change(busquedaFacturaProveedorn($("#numero_factura").val()));
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            if (detalles.length==0) {
              ...
              $('#list-busqueda').attr('style','overflow-y:scroll; height: 100px;');
              enabled();
            }
            //-------------------------------------
