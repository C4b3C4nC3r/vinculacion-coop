  ...
  public function update($item)
  {
    ...
    try {
      ...
      if ($row=$consulta->fetch()) {
        ...
        // code...
        $consulta=$this->db->connect()->prepare("UPDATE producto SET id_categoria_producto=:id_categoria_producto,codigo_producto=:codigo_producto,producto=:producto,descripcion=:descripcion,foto=:foto,precio=:precio,estado=:estado WHERE id_producto=:id_producto");//actualizar
        try {
          $consulta->execute(['id_producto'=>$item['id_producto'],'id_categoria_producto'=>$item['id_categoria_producto'],'codigo_producto'=>$item['codigo_producto'],'producto'=>$item['producto'],'descripcion'=>$item['descripcion'],'foto'=>$item['foto'],'precio'=>$item['precio'],'estado'=>$item['estado']]);
